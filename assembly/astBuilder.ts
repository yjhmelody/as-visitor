...
export class ASTBuilder extends BaseVisitor {
  ...
  visitIntegerLiteralExpression(node: IntegerLiteralExpression): void {
      this.sb.push(utils.i64_to_string(node.value));
  }

  visitStringLiteral(str: string, singleQuoted = false): void {
      this.sb.push("\"");
  ...
  protected visitFunctionCommon(node: FunctionDeclaration): void {
      var sb = this.sb;
      this.visitIdentifierExpression(node.name);
