...
export class BaseTransformVisitor extends AbstractTransformVisitor<Node> {
  ...
  visitArrayLiteralExpression(node: ArrayLiteralExpression): ArrayLiteralExpression {
      node.elementExpressions = this.visit(node.elementExpressions) as Expression[];
      return node;
  }
  ...
  visitLiteralExpression(node: LiteralExpression): LiteralExpression {
      switch (node.literalKind) {
          ...
          default:
              // @ts-ignore
              throw new Error("Invalid LiteralKind: " + node.literalKind.toString());
      }
  }
  ...
  visitStringLiteral(str: string, singleQuoted = false): string { return str; }
  ...
  visitClassDeclaration(node: ClassDeclaration, isDefault = false): ClassDeclaration {
      node.name = this.visit(node.name) as IdentifierExpression;
  ...
  visitEnumDeclaration(node: EnumDeclaration, isDefault = false): EnumDeclaration {
      node.name = this.visit(node.name) as IdentifierExpression;
